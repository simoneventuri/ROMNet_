...
class inputdata(object):
    ...
    def __init__(self, WORKSPACE_PATH):
        ...
        FileName                 = self.path_to_data_fld+'/../Orig/CleanVars_ToRed.csv'
        self.input_vars_all      = pd.read_csv(FileName, delimiter=',', header=None).to_numpy()[0,:]
        self.input_vars          = {'Encoder': {'FNN': list(self.input_vars_all)}, 
                                    'Decoder': {'FNN': ['x'+str(ix+1) for ix in range(self.NRODs)]}}
        ...
        self.norm_input_flg      = {'Encoder': {'FNN': True}, 
                                    'Decoder': {'FNN': False}}                                                   # Dictionary Containing Flags for Normalizing Input Data for each Component
        self.norm_output_flg     = True                                                                    # Flag for Normalizing Output Data
        self.data_preproc_type   = 'pareto'
        self.structure           = {'Encoder': {'FNN': ['Main']}, 
                                    'Decoder': {'FNN': ['Main']}}                                                # Dictionary Containing the Structure of the Network
        self.n_neurons           = {'Encoder': {'FNN': {'Main': np.array([40,30,20,self.NRODs])}}, 
                                    'Decoder': {'FNN': {'Main': np.array([20,30,40,self.n_outputs,self.n_outputs])}}}           # Dictionary Containing the No of Neurons for each Layer
        self.act_funcs           = {'Encoder': {'FNN': {'Main': ['relu','relu','relu','relu']}},
        ...
        self.softmax_flg         = {'Encoder': False,
                                    'Decoder': False}                                         # Dictionary Containing the Softmax Flag for each Sub-Component 
        ...
        #=======================================================================================================================================
        ### Training Quanties
        self.trainable_flg       = {'Encoder': 'all', 
                                    'Decoder': 'all'}                                                            # Dictionary Containing Instructions for Training Components ('all'/'none'/'only_last')
