...
class inputdata(object):
    ...
    def __init__(self, WORKSPACE_PATH, ROMNet_fld):
        ...
        # self.path_to_run_fld        = self.ROMNet_fld   + '/../0DReact_Isobaric_500Cases_Simple_POD_'+POD_NAME+'_Branch/'           # Path To Training Folder
        ...
        # self.path_to_data_fld       = self.ROMNet_fld   + '/../Data/0DReact_Isobaric_500Cases_Simple/'+str(self.NRODs)+'PC/OneByOne/POD_'+POD_NAME+'/Branch/'           # Path To Training Data Folder 
        ...
        self.ProbApproach        = 'Deterministic'                                                        # Probabilistic Technique for Training the BNN (if Any)
        self.InputVars           = ['PC_'+str(iPC+1) for iPC in range(self.NRODs)]                                                                 # List Containing the Input Data Column Names 
        self.OutputVars          = ['POD_'+str(i_mode+1) for i_mode in range(self.n_modes)]+['C']                                                              # List Containing the Output Data Column Names
        self.TransFun            = None#{'log': ['t']} 
        self.NormalizeInput      = True                                                                   # Flag for Normalizing Input Data
        self.Layers              = [np.array([64,64,64,64,self.n_modes+1]), np.array([64,64,64,self.n_modes+1])]                                  # List Containing the No of Neurons per Each NN's Layer
