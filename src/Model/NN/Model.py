...
def NNBranch(InputData, normalized, NNName, Idx):
    ...
    for iLayer in range(NLayers):
        ...
        # if (iLayer < NLayers-1):
        #     hiddenVec.append(layers.Dropout(InputData.BranchDropOutRate, input_shape=(NNLayers[iLayer],))(hiddenVec[-1]))          
...
def NNTrunk(InputData, normalized, NNName, Idx):
    ...
    for iLayer in range(NLayers):
        ...
        hiddenVec.append(layers.Dense(units=NNLayers[iLayer],
                                ...
                                name=LayerName)(hiddenVec[-1]))
        # if (iLayer < NLayers-1):
        #     hiddenVec.append(layers.Dropout(InputData.TrunkDropOutRate, input_shape=(NNLayers[iLayer],))(hiddenVec[-1]))          
...
class model:    
    ...
    def __init__(self, InputData, PathToRunFld, TrainData, ValidData):
        ...
        if (InputData.DefineModelIntFlg > 0):
            ...
            ### Branches
            #outputBranch       = NNBranch(InputData, input_, 'Branch', 0)

            ### Final Layer
            output2            = layers.Dense(units=self.NVarsy,
                                              activation='linear',
                                              use_bias=True,
                                              kernel_initializer='glorot_normal',
                                              bias_initializer='zeros',
                                              name='FinalScaling')(input_)   
            ...
            self.Model      = keras.Model(inputs=[input_], outputs=[output2] )
            #---------------------------------------------------------------------------------------------------------------------------
    ...
    def train(self, InputData):
        ...
        print('xTrain = ', self.xTrain.to_numpy())
        print('yTrain = ', self.yTrain.to_numpy())

        History       = self.Model.fit(self.xTrain.to_numpy(), self.yTrain.to_numpy(), 
                                       batch_size=InputData.MiniBatchSize, 
                                       validation_data=(self.xValid.to_numpy(), self.yValid.to_numpy()), 
                                       verbose=1, 
