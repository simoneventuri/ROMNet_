...
def load_weights_from_hdf5_group_by_name(f, layers, skip_mismatch=False):
	...
	# New file format.
	layer_names = load_attributes_from_hdf5_group(f, 'layer_names')
	#print( "[ROMNet - hdf5_format.py            ]:   layer_names = ", layer_names )
	...
	# We batch weight value assignments in a single backend call which provides a speedup in TensorFlow.
	weight_value_tuples = []
	for k, name in enumerate(layer_names):
		g = f[name]
		weight_names  = load_attributes_from_hdf5_group(g, 'weight_names')
		weight_values = [np.asarray(g[weight_name]) for weight_name in weight_names]
		#print( "[ROMNet - hdf5_format.py            ]:     weight_names  = ", weight_names )
		#print( "[ROMNet - hdf5_format.py            ]:     weight_values = ", weight_values )
		...
		for layer in index.get(name, []):
			...
			if len(weight_values) != len(symbolic_weights):
				if skip_mismatch:
					...
					print( '[ROMNet - hdf5_format.py            ]:     Skipping loading of weights for layer {}'.format(layer.name) + ' due to mismatch in number of weights ({} vs {}).'.format(len(symbolic_weights), len(weight_values)))
					continue
			...
			for i in range(len(weight_values)):
				if backend.int_shape(symbolic_weights[i]) != weight_values[i].shape:
					if skip_mismatch:
						...
						print( '[ROMNet - hdf5_format.py            ]:     Skipping loading of weights for layer {}'.format(layer.name) + ' due to  mismatch in shape ({} vs {}).'.format(symbolic_weights[i].shape, weight_values[i].shape))
						continue
					...
					print( "[ROMNet - hdf5_format.py            ]:     Loaded Weights for Layer ", name )
