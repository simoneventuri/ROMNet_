...
#=======================================================================================================================================
class NonLinear_DeepONet(System_of_Components):

    # ---------------------------------------------------------------------------------------------------------------------------
    def __init__(self, InputData, name, norm_input, stat_input, layers_dict=[], layer_names_dict=[]):
        super(NonLinear_DeepONet, self).__init__()
        ...
        for component_name in self.structure[self.name]:
            ...
            if  ('branch' in component_name.lower()):
                ...
                self.decoder_names.append(component_name)
    ...
    def call(self, inputs, layers_dict, training):
        ...
        for i_branch in range(self.n_branches): 
            ...
            decoder_name = self.decoder_names[i_branch] 
            ...
            if (self.n_branch_out == None) or (self.n_branch_out == self.n_trunk_out):
                ...
                output_compons   = tf.keras.layers.Multiply()([y, y_trunk_vec[i_trunk]]) 
                ...
                output_compons   = tf.keras.layers.Multiply()([alpha_vec, y_trunk_vec[i_trunk]]) 
                output_compons   = tf.keras.layers.Multiply()([output_compons,  d])            
                output_compons   = tf.keras.layers.Concatenate(axis=1)([output_compons, c])
            ...
            output_      = self.components[decoder_name].call(output_compons, layers_dict, None, training=training)
