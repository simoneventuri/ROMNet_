...
for iIC in range(NICs):
    ...
    for iShifts in range(NShifts):
        ...
        if (iShifts==0):
            ...
            FileName  = OutputDir+'/Orig/'+DirName+'/ext/y.csv.1'
            Data_0    = pd.read_csv(FileName)
            indx      = int( len(Data_0) / NShifts * iShifts )
            ...
            Y0        = Data_0[gas.species_names].iloc[indx].to_numpy()
            T0        = Data_0['T'].iloc[indx]
            ...
            indx      = int( len(Data_0) / NShifts * iShifts )-1
