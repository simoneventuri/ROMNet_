...
SpeciesVec         = ['H2','H','O','O2','N','N2']# 'NH', 'OH', 'NO']
        ...
        for itry in range(1):
            ...
            if (itry == 0):
                ...
                if (EqRatio0Exts):
                    ...
                    #SpecDict =  {'H2': 0.1427, 'H': 0.17149999999999999, 'O': 0.2633, 'O2': 0.0671, 'N': 0.9077000000000001, 'N2': 0.43609999999999993}
                    gas.X    = SpecDict

                    print('Pressure = ', gas.P, 'Pa; Temperature = ', gas.T, 'K; Mass Fractions = ', gas.Y)

            else:

                n_lines  = yMat.shape[0]
                idx      = np.random.randint(0, high=n_lines)
                tNow     = 0.#tVecFinal[idx]
                P0       = ICs[iIC,0]
                T0       = yMat[idx,1]
                gas.Y    = np.maximum(yMat[idx,2:], 0.)

                print('idx = ', idx, '; Pressure = ', P0, 'Pa; Temperature = ', T0, 'K; Mass Fractions = ', gas.Y)
            ...
            tVec     = np.concatenate([np.logspace(-10., np.log(.1), NPerT0)])
            ...
            if (Integration == 'Canteras'):
                ...
                output           = solve_ivp( IdealGasConstPressureReactor_SciPY, (tVec[0],tVec[-1]), y0, method=SOLVER, t_eval=tVec, atol=atol, max_step=1.e-3 )
                it0              = 0
                tVecFinal        = output.t
                HRVec            = []
