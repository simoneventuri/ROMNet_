...
########################################################################################################################
DataDir            = os.path.join(WORKSPACE_PATH, 'ROMNet/Data/MSD_100Cases/')
...
for VarName in Vars:
    ...
    if (DRType == 'All'):
        ...
        for iC in range(n_ics):
            ...
            #if (np.abs( (DataTemp[0] - DataTemp[len(Data)-1])/DataTemp[0] ) > 1.e-30):
            DataICTemp  = Data[[Vars[iVar] for iVar in range(NVars)]].iloc[0]
    ...
    DataInput  = DataNoZero[['t'] + Vars_Trunk]
    ...
    DataOutput = DataNoZero[['t'] + Vars_Trunk]
